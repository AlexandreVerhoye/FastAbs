...
struct CoachAdvisorTests {
    ...
    private func record(
        ...
        stored.plannedActiveDuration = 420
    ...
    func adaptationsAreAlwaysExplained() {
        ...
        // One-directional on purpose: the coach also steers the *selection* —
        // reaching for the job the week has skipped, standing off yesterday's
        // movements — and those are worth a sentence even though they leave the
        // settings untouched. What must never happen is the reverse.
        for records in [
            ...
            let recipe = CoachAdvisor(calendar: calendar)
                .recipe(records: records, base: base, now: date(13))
            if recipe.isAdapted {
                #expect(recipe.hasRationale, "an adjustment went unexplained")
            }
        }
    }

    @Test("An adjustment that could not happen is not announced")
    func impossibleAdjustmentsStaySilent() {
        // Coming back after a fortnight normally buys a shorter, gentler
        // session. At five minutes and the gentlest level there is nothing left
        // to give, and a coach who says "on repart sur 5 min" when the athlete
        // already asked for five minutes has started narrating rather than
        // coaching.
        let base = TestSupport.preferences(durationMinutes: 5, difficulty: .beginner)
        let recipe = CoachAdvisor(calendar: calendar)
            .recipe(records: [record(day: 1, difficulty: .beginner)], base: base, now: date(13))

        #expect(recipe.preferences.durationMinutes == 5)
        #expect(recipe.preferences.difficulty == .beginner)
        #expect(!recipe.rationale.contains { $0.contains("Reprise") })
    }

    @Test("A week away is met with a shorter, gentler session")
    func timeAwayShortensTheComeback() {
        let base = TestSupport.preferences(durationMinutes: 12)
        let recipe = CoachAdvisor(calendar: calendar)
            .recipe(records: [record(day: 1)], base: base, now: date(13))

        #expect(recipe.preferences.durationMinutes == 7)
        #expect(recipe.preferences.difficulty == .beginner)
        #expect(recipe.rationale.contains { $0.contains("Reprise en douceur") })
    }

    @Test("A fortnight of easy sessions does not outrank a fortnight away")
    func theBreakOutranksTheTrend() {
        // Both are true at once, and firing both would hand someone coming back
        // from a break a longer, harder session on the strength of how
        // three-week-old sessions felt.
        let records = (1...3).map { record(day: $0, effort: .easy) }
        let recipe = CoachAdvisor(calendar: calendar)
            .recipe(records: records, base: TestSupport.preferences(), now: date(20))

        #expect(recipe.preferences.difficulty == .beginner)
        #expect(!recipe.rationale.contains { $0.contains("faciles") })
    }

    @Test("Six days running buys a lighter day, not a lecture")
    func aLongStreakLightensTheSession() {
        let records = (7...12).map { record(day: $0, effort: .right) }
        let recipe = CoachAdvisor(calendar: calendar)
            .recipe(
                records: records,
                base: TestSupport.preferences(durationMinutes: 12),
        ...
        #expect(recipe.preferences.durationMinutes == 9)
        #expect(recipe.preferences.extraRecovery)
        #expect(recipe.rationale.contains { $0.contains("d’affilée") })
    }

    @Test("At the gentlest level, hard sessions buy recovery instead")
    func theFloorGivesRecoveryWhenItCannotGiveLevel() {
        // Nothing below beginner. Saying nothing here is how an athlete at the
        // gentlest setting learns the app has run out of answers.
        let records = [
            record(day: 11, difficulty: .beginner, effort: .hard),
            record(day: 12, difficulty: .beginner, effort: .hard)
        ]
        let recipe = CoachAdvisor(calendar: calendar)
            .recipe(
                records: records,
        ...
        #expect(recipe.preferences.difficulty == .beginner)
        #expect(recipe.preferences.extraRecovery)
        #expect(recipe.rationale.contains { $0.contains("Récupération renforcée") })
    }

    @Test("A session quit before it could count is still noticed")
    func abandonedTooEarlyToQualifyStillCounts() {
        // Quitting after ninety seconds leaves no qualifying record at all,
        // which is precisely the case the athlete most needs the coach to see.
        // Reading only the qualifying history made the shortening rule fire for
        // everyone except the people it was written for.
        let records = (11...12).map { day -> WorkoutRecord in
            let stored = record(day: day)
            stored.wasCompleted = false
            stored.activeDuration = 40
            return stored
        } + [record(day: 9)]
    ...
    @Test("The selection explains itself too")
    func steeringTheChoiceIsSpokenAloud() {
        // The engine already stands off yesterday's movements and reaches for
        // the week's missing job. Without a sentence, both are invisible and
        // the session reads as a reshuffled list.
        let records = [record(day: 11, exercises: ["forearm-plank"])]
        let recipe = CoachAdvisor(calendar: calendar)
            .recipe(records: records, base: TestSupport.preferences(), now: date(12))

        #expect(recipe.rationale.contains { $0.contains("manque à votre semaine") })
    }

    @Test("Steering the choice is not the same as changing the settings")
    func selectionAdviceIsNotAnAdaptation() {
        // Some job is nearly always missing from a week still in progress, so a
        // badge driven by the rationale would be lit permanently and therefore
        // mean nothing. The badge answers "is this the session I asked for";
        // the rationale answers "why this one".
        let records = [record(day: 12, exercises: ["forearm-plank"])]
        let base = TestSupport.preferences(focusZones: [.lowerAbs])
        let recipe = CoachAdvisor(calendar: calendar)
            .recipe(records: records, base: base, now: date(12, hour: 20))

        #expect(recipe.preferences == base)
        #expect(!recipe.isAdapted)
        #expect(recipe.hasRationale)
    }

    @Test("Every sentence the coach writes is a sentence")
    func rationaleReadsAsFrench() {
        let histories: [[WorkoutRecord]] = [
            (10...12).map { record(day: $0, effort: .easy) },
            [record(day: 11, effort: .hard), record(day: 12, effort: .hard)],
            (6...12).map { record(day: $0, exercises: ["forearm-plank"]) },
            [record(day: 1)],
            (11...12).map { day -> WorkoutRecord in
                let stored = record(day: day)
                stored.wasCompleted = false
                return stored
            }
        ...
        for history in histories {
            for difficulty in WorkoutDifficulty.allCases {
                let recipe = CoachAdvisor(calendar: calendar).recipe(
                    records: history,
                    base: TestSupport.preferences(difficulty: difficulty),
                    now: date(13)
                )
                for line in recipe.rationale {
                    #expect(line.hasSuffix("."), "unpunctuated: \(line)")
                    #expect(!line.contains("'"), "straight apostrophe: \(line)")
                    #expect(!line.contains("  "), "double space: \(line)")
                    // Several lines legitimately open on a number of minutes.
                    let first = line.first
                    #expect(
                        first?.isUppercase == true || first?.isNumber == true,
    ...
    @Test("The level never drifts more than one step, whatever the history")
    func theLevelStaysWithinOneStepOfTheAthletesChoice() {
        let histories: [[WorkoutRecord]] = [
            (3...12).map { record(day: $0, effort: .easy) },
            (3...12).map { record(day: $0, effort: .hard) },
            [record(day: 1)],
            (7...12).map { record(day: $0, effort: .easy) }
        ]

        for history in histories {
            for difficulty in WorkoutDifficulty.allCases {
                let base = TestSupport.preferences(difficulty: difficulty)
                let recipe = CoachAdvisor(calendar: calendar)
                    .recipe(records: history, base: base, now: date(13))
                #expect(
                    abs(recipe.preferences.difficulty.rawValue - difficulty.rawValue) <= 1,
                    "\(difficulty) drifted to \(recipe.preferences.difficulty)"
                )
            }
        }
    }
