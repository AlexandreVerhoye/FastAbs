...
struct ProgressVisualTests {
    @Test("With no history the tab invites rather than reporting zeros")
    func firstRunIsAnInvitation() {
        guard let rendered = VisualProbe.require(
            ProgressFirstRunView(start: {}),
            width: 360, height: 520, "first run"
        ) else { return }

        #expect(!rendered.isBlank)
        #expect(rendered.paintedRatio > 0.3, "the first-run screen barely renders")
        // The whole point of the state is that it carries an action.
        #expect(
            rendered.contains(PixelColor(Color.haraCoral), tolerance: 0.22, minimumCoverage: 0.02),
            "the one action is not visible"
        )
    }

    @Test("A period with no sessions draws no chart at all")
    func anEmptyPeriodDrawsNoPlot() {
        // Axis labels around an empty plot were the most discouraging thing on
        // this tab, and the first thing a new athlete saw.
        let empty = VisualFixture.days(count: 30)
        let filled = VisualFixture.days(count: 30, activeSecondsByOffset: [10: 600, 20: 900])

        func card(_ days: [WorkoutHistoryDay]) -> ActivityChartCard {
            ActivityChartCard(
                days: days,
        ...
        guard
            let blank = VisualProbe.require(card(empty), width: 340, height: 320, "empty period"),
            let drawn = VisualProbe.require(card(filled), width: 340, height: 320, "period with sessions")
        else { return }

        #expect(!blank.isBlank, "the card still has to say something")
        // Measured against each other rather than against zero: the card's own
        // chrome puts a little warm colour on screen in both states, and what
        // matters is that only real days reach the plot.
        let tint = PixelColor(Color.haraCoral)
        #expect(
            drawn.count(of: tint, tolerance: 0.25) > blank.count(of: tint, tolerance: 0.25) * 3,
            "an empty period paints as much as a busy one"
        )
    }
    ...
    func challengeRingGrowsWithProgress() {
        ...
        for current in [0, 3, 6, 12] {
            guard let rendered = VisualProbe.require(
                ChallengeCard(challenge: VisualFixture.challenge(current: current, target: 12), tint: .haraCoral),
    ...
    func completedChallengeLooksDifferent() {
        guard
            let partial = VisualProbe.require(
                ChallengeCard(challenge: VisualFixture.challenge(current: 8, target: 12), tint: .haraCoral),
                width: 340, height: 130, "partial challenge"
            ),
            let complete = VisualProbe.require(
                ChallengeCard(challenge: VisualFixture.challenge(current: 12, target: 12), tint: .haraCoral),
        ...
        #expect(
            complete.count(of: PixelColor(Color.haraCoral), tolerance: 0.22)
                > partial.count(of: PixelColor(Color.haraCoral), tolerance: 0.22)
        )
    }
    ...
    func challengeCardUsesItsTint() {
        guard
            let coral = VisualProbe.require(
                ChallengeCard(challenge: VisualFixture.challenge(current: 9, target: 12), tint: .haraCoral),
                width: 340, height: 130, "coral challenge"
            ),
            let blue = VisualProbe.require(
                ChallengeCard(challenge: VisualFixture.challenge(current: 9, target: 12), tint: .haraBlue),
                width: 340, height: 130, "blue challenge"
            )
        else { return }

        #expect(coral.contains(PixelColor(Color.haraCoral), tolerance: 0.2, minimumCoverage: 0.01))
        #expect(blue.contains(PixelColor(Color.haraBlue), tolerance: 0.2, minimumCoverage: 0.01))
    ...
    func restDaysReadAsRest() {
        ...
        #expect(
            active.count(of: PixelColor(Color.haraCoral), tolerance: 0.3)
                > rest.count(of: PixelColor(Color.haraCoral), tolerance: 0.3) * 4
        )
    }
    ...
    func firstWeekIsNeutral() {
        ...
        #expect(!rendered.isBlank)
        #expect(rendered.contains(PixelColor(Color.haraBlue), tolerance: 0.28, minimumCoverage: 0.001))
    }
    ...
    func degenerateChallengeStillRenders() {
        guard let rendered = VisualProbe.require(
            ChallengeCard(challenge: VisualFixture.challenge(current: 0, target: 0), tint: .haraCoral),
