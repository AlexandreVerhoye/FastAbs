...
struct WorkoutHistoryAnalytics {
    /// The least work a session must contain to count as one at all.
    ///
    /// Two minutes, because a finished five-minute session at the gentlest
    /// level is only about three minutes of actual work — the rest of it is
    /// recovery — and a floor above that would throw away sessions the athlete
    /// completed in full.
    static let minimumActiveDuration = 120
    static let minimumCompletionRatio = 0.7
    ...
    /// Whether a record counts as a session the athlete actually did.
    ///
    /// Measured against the work the plan asked for, never against its
    /// wall-clock length. Those are different quantities — most of a gentle
    /// session is recovery — and comparing one to the other is what made this
    /// return false for very nearly every completed session: a seven-minute
    /// beginner plan contains about three minutes of work, so the ratio came
    /// out around 0.42 and could never clear a 0.75 bar. Every screen that
    /// reads the history goes through here, so streaks, charts, badges,
    /// personal records and the coach's own memory were all quietly empty.
    func isQualifying(_ record: WorkoutRecord) -> Bool {
        guard record.activeDuration >= Self.minimumActiveDuration else { return false }
        // Written before the work total was tracked: the absolute floor is the
        // only thing such a record can still be judged on.
        guard record.plannedActiveDuration > 0 else { return true }
        return Double(record.activeDuration) / Double(record.plannedActiveDuration)
            >= Self.minimumCompletionRatio
    }
